...
def create_theme(request):
    ...
    return render(
        request, 'boards/create_theme.html',
...
def list_themes(request):
    ...
    return render(
        request, 'boards/list_themes.html',
...
def edit_theme(request, id):
    ...
    if edit_theme_form.is_valid():
        ...
        return redirect('boards:list_themes')
    return render(
        request, 'boards/edit_theme.html',
        context={ 'edit_theme_form': edit_theme_form, 'id':id }
    )
...
def post_comments(request, theme_id):
    saved_comment = cache.get(f'saved-comment-theme_id={theme_id}-user_id={request.user.id}', '')
    post_comment_form = forms.PostCommentFrom(request.POST or None, initial={ 'comment': saved_comment})
    ...
    if post_comment_form.is_valid():
        ...
        cache.delete(f'saved-comment-theme_id={theme_id}-user_id={request.user.id}')
        return redirect('boards:post_comments', theme_id=theme_id)
    return render(
        request, 'boards/post_comments.html',
        context ={
...
def save_comment(request):
    if request.is_ajax:
        ...
        if comment and theme_id:
            cache.set(f'saved-comment-theme_id={theme_id}-user_id={request.user.id}', comment)
